...
class Sync {
  ...
  static Future<void> fullSyncPostData(AppDatabase database) async {
    try {
      log(
        '🔄 [PHASE 1] Starting full sync post data - Validating authentication...',
      );
      // Phase 1: Validate authentication
      final authData = await _validateAuthentication();
      log(
        '✅ [PHASE 1] Authentication validated - User ID: ${authData['userId']}',
      );
  ...
  static Future<Map<String, dynamic>> _collectUnsyncedData(
    ...
    if (unsyncedVaccinations.isNotEmpty) {
      ...
      for (final v in unsyncedVaccinations.take(3)) {
        log(
          '  💉 Vaccination: uuid=${v['uuid']}, vaccineUuid=${v['vaccineUuid']}, farmUuid=${v['farmUuid']}, livestockUuid=${v['livestockUuid']}',
        );
      }
    }
    ...
    final unsyncedBirthEvents = await eventsRepository
        .getUnsyncedBirthEventsForApi();

    log('📤 Fetching unsynced aborted pregnancies for API...');
    final unsyncedAbortedPregnancies = await eventsRepository
        .getUnsyncedAbortedPregnanciesForApi();
    ...
    if (unsyncedFarmUsers.isNotEmpty) {
      log(
        '📧 ${unsyncedFarmUsers.length} farm user(s) will be synced - invitation emails will be sent',
      );
    }
  ...
  static Future<SyncUnsyncedSummary> getUnsyncedSummary(
    ...
    final abortedPregnancies = await eventsRepository
        .getUnsyncedAbortedPregnanciesForApi();
    ...
    final inseminations = await eventsRepository
        .getUnsyncedInseminationsForApi();
  ...
  static Future<void> _markItemsAsSynced(
    ...
    log(
      '🔍 [DEBUG] syncedData[\'syncedLivestock\']: ${syncedData['syncedLivestock']}',
    );
    log(
      '🔍 [DEBUG] syncedData[\'syncedLivestock\'] type: ${syncedData['syncedLivestock']?.runtimeType}',
    );
    log('🔍 [DEBUG] syncedData keys: ${syncedData.keys.toList()}');

    if (syncedLivestockUuids.isNotEmpty) {
      log(
        '🔍 [DEBUG] Calling markLivestockAsSynced with ${syncedLivestockUuids.length} UUIDs',
      );
      final livestockRepository = LivestockRepository(database);
      final result = await livestockRepository.markLivestockAsSynced(
        syncedLivestockUuids,
      );
      log(
        '  ✅ Marked ${syncedLivestockUuids.length} livestock as synced (result: $result)',
      );
    } else {
      log('⚠️ [DEBUG] No livestock UUIDs to mark as synced');
      log(
        '⚠️ [DEBUG] syncedData[\'syncedLivestock\'] is: ${syncedData['syncedLivestock']}',
      );
    }
    ...
    if (syncedLogs != null && syncedLogs.isNotEmpty) {
      ...
      final syncedAbortedPregnancies = _extractUuids(
        syncedLogs['abortedPregnancies'],
      );
      if (syncedAbortedPregnancies.isNotEmpty) {
        await eventsRepository.markAbortedPregnanciesAsSynced(
          syncedAbortedPregnancies,
        );
        log(
          '  ✅ Marked ${syncedAbortedPregnancies.length} aborted pregnancies as synced',
        );
      }
    ...
    final syncedVaccineUuids =
        (syncedData['syncedVaccines'] as List<dynamic>?)
            ?.map((item) => item['uuid'] as String)
            .toList() ??
        [];
  ...
  static Future<void> _storeUserSpecificData(
    ...
    try {
      ...
      if (rawUserSpecificData is Map<String, dynamic>) {
        ...
        log(
          '⚠️ userSpecificData is a List (array) instead of Map - converting to empty map',
        );
        ...
        log(
          '⚠️ Unexpected userSpecificData type: ${rawUserSpecificData.runtimeType}, defaulting to empty map',
        );
        userSpecificData = const {};
      }
    } catch (e, stackTrace) {
    ...
    log(
      '📋 [SYNC] Processing user-specific data for role: ${userType.isEmpty ? "unknown" : userType}',
    );
    ...
    if (userType == 'farmer') {
      ...
      if (rawLogs is Map<String, dynamic>) {
        ...
        log(
          '⚠️ Unexpected logs payload type: ${rawLogs.runtimeType}, defaulting to empty map',
        );
        logs = const {};
      }
      ...
      // Store Invited Extension Officers
      final invitedExtensionOfficers =
          (userSpecificData['invitedExtensionOfficers'] as List?)
              ?.cast<Map<String, dynamic>>() ??
          const <Map<String, dynamic>>[];
      await database.extensionOfficerDao.syncInvitedOfficers(
      ...
      log(
        '  👷 [SYNC] Storing field worker (farm invited user/extension officer/vet) data...',
      );
      ...
      final farmUserData =
          userSpecificData['farmUser'] as Map<String, dynamic>?;
      if (farmUserData != null) {
        try {
          ...
          log(
            '  📋 Farm User Profile: $firstName $lastName ($roleTitle) - Assigned to ${farmUuids.length} farm(s)',
          );
        } catch (e, stackTrace) {
      ...
      log(
        '  👨‍💼 [SYNC] System user - uses admin endpoints (no local data storage)',
      );
    } else {
      log(
        '  ⚠️ [SYNC] Unknown or missing user type: "$userType" - no user-specific data stored',
      );
    }
  }
}
